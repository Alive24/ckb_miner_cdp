...
- **Collateralized**: [Redemption](./Mechanisms/Redemption) is backed by the whole protocol collectively instead of the actual miner;
...
2. **David accepts the recruiting**:
  ...
  - Receives 1,960 CKB funding immediately (2,000 × 0.98)
...
- **Initial state**: 2,000 CKB collateral, 1,000 CKB debt → ICR = 200%
- **Interest accrual**: Alice's debt grows daily at 10% APR
- **After interest accrual**: 2,000 CKB collateral, 1,080 CKB debt → ICR = 185%
...
- Each day, 5% of debt is liquidated: 2,800 × 5% = 140 CKB debt
- Collateral used: 140 × MCR (e.g., 150%) = 210 CKB
- After liquidation:
...
Holders of [mCKB](./Tokens#mckb-mining-contract-token) tokens can redeem them for CKB at a 1:1 ratio from the protocol pool. Redemption takes 30 days by default without additional fee while quicker options (instant, 1 day, 7 days, 14 days, 30 days) with varying fee rates are also available.
